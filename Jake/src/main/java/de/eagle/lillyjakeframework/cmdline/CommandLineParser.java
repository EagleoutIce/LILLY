 ...
 * entsprechend aufzuarbeiten und zu verarbeiten
 * Er besitzt als überarbeitung der Cpp-Variante nun Kenntniss
 * aller Einstellungen und kann auf der Basis eine Plausibilitätsprüfung 
 * durchführen und Fehler früher offenbaren.
 * Er wird allerdings keine grundlegende Analyse (wie ist die Datei gültig?)
 * durchgeführt - dies obliegt dann der Interpretation der Einstellungen.
 * Weiter interessiert es den CommandLineParser auch nicht, ob die
 * Einstellungen über eine Datei weiter geladen werden,
 * ihn beschäftigt nur, was über die Kommandozeile kommt
 * 
 * @implNote es gibt noch eine weitere Neuerung zum Cpp-Parser.
 *           während dieser CmdLine-Argumente durch Konfigurationen
 *           überschrieben hat so haben im Java modell die Kommandozeilen
 *           Argumente stets den Vorrang, sagt also die Befehlszeil
 *           name = A und eine Konfigurationsdatei name = B so wird
 *           name = A übernommen!
 * 
 * @author Florian Sihler
 * @version 1.0.10
...
public class CommandLineParser {
     ...
     * @param settings die Einstellungen - sie werden überschrieben. 
     *                 Alle Einstellungen die hier gese tzt werden 
     *                 werden automatisch auf immutable gesetzt
     *                 und lassen sich somit nichtmehr überschreiben
     * 
    ...
    protected static int parse_next_arg(String[] args, int current_arg, Settings settings){
        ...
        if(carg.charAt(0) != ARGUMENT_PATTERN){ // einzelnes Argument
            ...
            if(current_arg == args.length-1){
                writeLoggerError("Die Option \"" + carg + "\" benötigt ein Argument! Dieses wurde nicht geliefert!","Parser");
            ...
            if(carg.contains(ADD_ASSIGN_PATTERN)) {  // Ist eine Addition
                ...
                writeLoggerDebug2("Weise zu: \"" + strip_modifications(carg) + "\" += \"" + args[current_arg] + "\" (feedback: " + b + ")", "CmdLineP.");
