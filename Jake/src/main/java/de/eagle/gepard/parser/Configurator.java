 ...
 *  a = b
 * Er fügt gegenüber dem @see Tokenizer weitere Optionen wie die Liste hinzu:
 *  a += b 
 */
public class Configurator {
     ...
     * @throws FileNotFoundException Wenn die Datei ungültig/beschädigt/nonexistent ist
     */
    ...
    /**
     * Verarbeitet Einstellungen auf Basis der zuvor geladenen Daten (input_path/inputStream)
     * @param settings die zu verwendene Einstellungen sie werden *überschrieben*
     * @param add_unknown mit true werden auch unbekannte Einstellungen hinzugefügt dies
     *                      ÜBERSCHREIBT die policy der übergebenen Settings
        ...
        for (TokenMatch m : this._tokenizer) {
            ...
            if (m.getMatchings().length == 4) {
                ...
                if (settings.containsKey(matchings[LHS]) || add_unknown) { // Test ansich redundant da Settings policy 
                    writeLoggerDebug2("Zuweisung gefunden: " + matchings[LHS] + " !" + matchings[OPT] + "! " + matchings[RHS], "config");
                    ...
                    switch(matchings[OPT]){ // Unterscheide nach Operation
                        ...
                        case "+=":
                            ...
                            if(old==null) {
                                ...
                                writeLoggerDebug1("Neue Liste erstellt: " + settings.get(matchings[LHS]).toString(), "config");
                            } else {
                                if(settings.get(matchings[LHS]).type.equals(eSetting_Type.IS_TEXTLIST)) {
                                    writeLoggerDebug1("Listenaddition gefunden", "config");
                                    settings.add(matchings[LHS],matchings[RHS]);
                                } else {
                                    writeLoggerDebug1("Listenaddition gefunden, aber keine Liste -> normale konkatenation", "config");
                            ...
                            break;
                        default:
                            writeLoggerWarning("Die Operation: " + matchings[OPT] + " in: \"" + matchings[ALL] + "\" ist ungütig!","config");
                    }
                } else
                writeLoggerWarning(COL_ERROR + "Die Zuordnung \"" + m.getStripped() + "\" ist ungültig" + COL_RESET, "config");
            } else
            writeLoggerWarning(COL_ERROR + "Die Zeile: " + m.getMatchings()[ALL] + " ist ungültig! " + COL_RESET, "config");
