...
\newcommand*{\blankcmd}[1]{\textcolor{Orchid}{\LILLYxlstTypeWriter\textbackslash\ignorespaces #1}}
\newcommand*{\blankenv}[1]{\textcolor{AppleGreen}{\LILLYxlstTypeWriter env@\ignorespaces #1}}
...
\DeclareDocumentCommand{\addcmd}{ O{1.0.0} m}{
    \index{#2@{\blankcmd{#2}\textsuperscript{\tiny\T{(v#1)}}}}%
}
\DeclareDocumentCommand{\addenv}{ O{1.0.0} m}{
    \index{#2@{\blankenvidx{#2}\textsuperscript{\tiny\T{(v#1)}}}}%
}
...
% Explain some libs like LILLYxLIST
% Make debug package <= FIX THIS
% Explain the xTABLES and xTABLESxMATERIAL
% Explain LAYOUT controller and how to make pages according to layout :D
% MAke the persitence library (2.1.0)
% Document the acronym package :D
% so many todo
% Infoboxes show a pdf snippet from another pdf :D
% Document the timetables
