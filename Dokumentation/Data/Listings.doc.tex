...
Die allgemeine TypeWriter-Schriftart wird übgrigens mithilfe von \blankcmd{LILLYxlstTypeWriter} auf \T{AnonymousPro} gesetzt (\textit{Sie wird auch hier für die Dokumentation verwendet}).
...
%
%
\presentCommand[2.0.0]{lstcomment}[\manArg{text}\cmdlist \blankcmd{lststring}\manArg{text}\cmdlist \blankcmd{lstnumber}\manArg{text}]
...
\isLanguageNameLoaded{java} // ::c:yields: :!:*\lstcomment{\isLanguageNameLoaded{java}}*:!:
...
%
%
\presentCommand[2.0.0]{lstkwone}[\manArg{text}\cmdlist \blankcmd{lstkwtwo}\manArg{text}\cmdlist\ldots \cmdlist \blankcmd{lstkwsix}\manArg{text}]
...
Registriert eine \T{Sprache} wie \T{java} mit der entsprechenden Listing-Sprache \T{lst-language} wie \T{lJava}. Die Signatur der Sprache wird in die Liste \T{sig-list}, der ledigliche Name (wie \T{java}) in die Liste \T{name-list} eingetragen. Konstruiert werden die \jmark[weiter oben]{mrk:reglangs} beschriebenen Befehle für die Sprache. Erschaffen wir uns einmal die Sprache \T{rubberduck}:
\begin{latex}
...
\prubberduck{Quack primus = new Quack();}
\end{latex}
...
%
%
\presentEnvironment[2.0.0]{presentlst}[\optArg{lst-args}\manArg{language}\cmdlist\secline\blankenv{plainlst}\optArg{lst-args}\manArg{language}\cmdlist\secline\blankenv{defaultlst}\optArg{lst-args}\manArg{language}]
