...
Das Laden des Pakets mit LILLY kann durch die Option \T{math} aktiviert werden (Standard) und durch \T{nomath} entsprechend deaktiviert. So sorgt das Deklarieren von:
\begin{latex*}
...
\presentCommand[2.2.0]{dist}[\manArg{math-Ausdruck}]
Setzt einen Ausdruck in doppelte Striche und kann so für Abstände eingesetzt werden: \clatex{:bmath:\\dist\{x\}:emath:} ergibt: $\dist{x}$.
...
Die komplexe Einheit \T{i} wird mit \blankcmd{i} zur Verfügung gestellt.
...
Hier befinden sich die für \la kreierten: \T{\blankcmd{enum}\manArg{items}} (\blankenv{enumerate} mit \newline\blankcmd{narrowitems}) und \T{\blankcmd{liste}\manArg{items}} (\blankenv{enumerate} mit römischen Zahlen und \newline\blankcmd{narrowitems}).
...
\presentCommand[1.0.7]{folge}[\optArg[a]{Folgenglied}\cmdlist\anothercmd[2.2.0]{folgek}\optArg[a]{Folgenglied}]
Setzt eine Folge, welche mit dem Index $n$ arbeitet: \blankcmd{folge} (\folge). Analog: \blankcmd{folgek} (\folgek).
...
\presentCommand[2.0.0]{ymark}[\optArg[y]{text}\manArg{PosY}\optArg[0.15]{linelength}]
