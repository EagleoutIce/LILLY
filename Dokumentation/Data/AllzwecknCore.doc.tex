...
\hypertarget{LILLYxFONTS}Diese Definitionen werden über die Bibliothek \LILLYxNOTExLibrary{LILLYxFONTS} zur Verfügung gestellt. Sie werden mit \LILLYxBOXxVersion{2.1.0} automatisch mit dem Einbinden von \LILLYxNOTExLibrary{LILLYxUTIL} geladen. Es kann eine der folgenden Optionen dem Paket oder Lilly übergeben werden, um die Dokumentschriftart zu ändern: \typesetList[T]{garamond,crimson,lcrimson,kpfont,overclock,pxfont,palatino}. Diese Dokumentation wurde durch folgenden Aufruf erzeugt:
\begin{latex*}
\documentclass[Dokumentation,kpfont]{Lilly}
\end{latex*}
Mit \LILLYxBOXxVersion{2.2.0} wird die Option \T{garamond} standardmäßig gesetzt. Wer die normale \LaTeX-Schriftart möchte kann hierzu die Option \T{defaultfont} übergeben um das Laden der Schriftart zu verhindern.
...
\presentCommand[1.0.0]{LILLYxlstTypeWriter}
Die allgemeine TypeWriter-Schriftart wird mithilfe von \blankcmd{LILLYxlstTypeWriter} auf \T{AnonymousPro} gesetzt (\textit{Sie wird auch hier für die Dokumentation verwendet}).
...
\presentCommand[2.2.0]{TypesetList}[\optArg[typesetVoid]{command}\manArg{ListName}\manArg{LastAnd}]
Funktioniert analog zu \blankcmd{typesetList}, gestattet aber das setzen des wortes zwischen den letzten beiden Elementen:
\begin{latex*}
\TypesetList[blankcmd]{Ich, mag, züüüge, so viele züüge}{oder}.
\end{latex*}
Ergibt: \TypesetList[blankcmd]{Ich, mag, züüüge, so viele züüge}{oder}.
...
\presentCommand[1.0.6][2.2.0]{LILLYcoloredSQ}[\optArg{Text}\manArg{Color}]
Zeigt die Farbe in einem formatierten Rechteck: \blatex{\\LILLYcoloredSQ\{bondiBlue\}} ergibt: \LILLYcoloredSQ{bondiBlue}. Mit \LILLYxBOXxVersion{2.2.0} kann ein Text übergeben werden: \blatex{\\LILLYcoloredSQ[\\tiny Hi]\{bondiBlue\}} ergibt: \LILLYcoloredSQ[\tiny Hi]{bondiBlue}.

%
%
%

\presentCommand[1.0.6][2.2.0]{LILLYxCOLORxRainbow}
Zeigt alle Farben die im aktuellen Profil verwendet werden mithilfe von \blankcmd{LILLYcoloredSQ} an:
\LILLYxCOLORxRainbow. Mit Version \LILLYxBOXxVersion{2.2.0} werden auch die entsprechenden Kurzbezeichner der jeweiligen Farbe in der Box angezeigt. \T{Ub*} steht hierbei für eine Zusatzübung, \T{Lin} für die Link-farbe, \T{Lid} für die dunklere Linkfarbe und \T{coA - coD} für die eingeführten \jmark[Paletten]{mrk:palettes}.
...
\LILLYxDemandPackage{amssymb}{Noch mehr Symbole}
...
Alle entsprechenden Befehle: \typesetList[stdloperation]{CONTROLLERS,DATA,GRAPHICS,FALLBACKS,HELPER,LISTINGS,MATHS,BEAMER,UTIL,CORE,PRESENTER}. Diese werden lediglich zu absoluten Pfaden aufgelöst, wenn \T{shell-escape} (\T{write-18}) aktiviert ist.
...
Dieses Paket erlaubt einen einfachen und standardisierten Zugriff für \T{.aux}-Dateien. Das Dokument muss zweimal kompiliert werden, damit die hier vermerkten Modifikationen gültig werden. Übrigens: Sollte aufgrund zu vieler Recorder dieser Fehler erscheinen: \say{No room for a new \blankcmd{write}}, so genügt es Lilly (oder \LILLYxNOTExLibrary{LILLYxRECORDER}) die Option \T{morewrites} zu übergeben!
