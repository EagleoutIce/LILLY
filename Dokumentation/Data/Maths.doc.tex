...
\chapter[Mathe \LILLYxBOXxVersion{\small 1.0.10}]{Mathe}
\TitleSUB{Einzelne Variationen und eine Menge Abkürzungen \hfill \LILLYxBOXxVersion{\small 1.0.10}}
\bigskip\newline
\renewcommand{\arraystretch}{1.5}
\elable{chp:MATH}An sich ändert LILLY nicht viel an der normalen Implementation der Mathewelt. Dieses Paket liegt hier: \begin{center}
...
Der verwendete Mathemodus lässt sich mithilfe des Befehls \clatex{:bs:LILLYxMathxMode} frei einstellen. Standardmäßig wird dieser Wert auf \emph{normal} gesetzt.
\begin{bemerkung}[Standalone-Math]
    Mit \LILLYxBOXxVersion{1.0.10} wurde die Mathe-Integration als eigenes Paket (\LILLYxNOTExLibrary{LILLYxMATH}) etabliert, welches sich auch eigenständig über \begin{latex}
...
Diese Definitionen befinden sich in der Datei: \T{Maths/\_LILLY\_MATHS\_OPERATORS}. Sie werden mit \LILLYxBOXxVersion{1.0.10} automatisch mit dem Einbinden von \LILLYxNOTExLibrary{LILLYxMATH} geladen.\medskip
%
%
%
...
\begin{center}
    \begin{tabular}{!{\VRule[1pt]}@{\hspace{1em}}l@{\hspace{1em}}|@{\hspace{1em}}c@{\hspace{1em}}!{\VRule[1pt]}}
        ...
        \clatex{:bs:overbar\{a\_1\} :bs:overbar\{a\_2\}} & \(\overbar{a_1}\overbar{a_2}\)\\\hline
        \clatex{:bs:overline\{a\_1\} :bs:overline\{a\_2\}} & \(\overline{a_1}\overline{a_2}\)\\
        \specialrule{1pt}{0pt}{0pt}
    \end{tabular}
\end{center}
%
...
\begin{center}
    \begin{tabular}{!{\VRule[1pt]}@{\hspace{1em}}l@{\hspace{1em}}|@{\hspace{1em}}c@{\hspace{1em}}!{\VRule[1pt]}}
        \specialrule{1pt}{0pt}{0pt}
        \clatex{:bs:sqrt[3]\{42\}} & \(\sqrt[3]{42}\)\\\hline
        \clatex{:bs:oldsqrt[3]\{42\}} & \(\oldsqrt[3]{42}\)\\
        \specialrule{1pt}{0pt}{0pt}
...
\begin{center}
    \begin{tabular}{!{\VRule[1pt]}@{\hspace{1em}}C{7cm}@{\hspace{1em}}|@{\hspace{1em}}C{2cm}@{\hspace{1em}}!{\VRule[1pt]}}
        \specialrule{1pt}{0pt}{0pt}
        \clatex{:bmath::bs:abs\{:bs:frac\{:bs:pi-x^2\}\}\{:bs:log 3x\}:emath:} & \(\displaystyle\abs{\frac{\pi - x^2}{\log 3x}}\)\\\hline
        \clatex{:bmath:|:bs:frac\{:bs:pi-x^2\}\{:bs:log 3x\}|:emath:} & \(\displaystyle|\frac{\pi - x^2}{\log 3x}|\)\\
        \specialrule{1pt}{0pt}{0pt}
    \end{tabular}\smallskip
\end{center}
%
...
%
%
\presentCommand[1.0.10]{dif}[\cmdlist \textbackslash dint\optArg[x]{Variable}]
...
Diese Definitionen befinden sich in der Datei: \T{Maths/\_LILLY\_MATHS\_SYMBOLS}. Sie werden mit \LILLYxBOXxVersion{1.0.10} automatisch mit dem Einbinden von \LILLYxNOTExLibrary{LILLYxMATH} geladen.\medskip
...
Auch die komplexe Einheit \i{} wird mit \blankcmd{i} zur Verfügung gestellt.\\
%
%
%
...
\presentCommand[1.0.10]{join}[\cmdlist \textbackslash leftouterjoin\cmdlist \textbackslash rightouterjoin\cmdlist \textbackslash fullouterjoin]
...
Diese Definitionen befinden sich in der Datei: \T{Maths/\_LILLY\_MATHS\_COMPATIBILITIES}. Sie werden mit \LILLYxBOXxVersion{1.0.10} automatisch mit dem Einbinden von \LILLYxNOTExLibrary{LILLYxMATH} geladen.\medskip

Hier werden einige Befehle eingerichtet, die entweder noch nicht zugeordnet wurden\({}^{\tiny \LILLYxBOXxVersion{\tiny 1.0.10}}\) oder während der Vorlesung (im Überlebenskampf :P) ins \T{eagleStudiPackage} eingebaut worden sind.
%
%
%
...
Für Ti\textit{k}Z gibt es noch die Befehle \blankcmd{crossAT\manArg{(PosX,PosY)}} (\raisebox{-0.35\baselineskip}{\tikz{\crossAT{(0,0)};}}\footnote{\T{\blankcmd{tikz}\{\blankcmd{crossAT}\{(0,0)\};\}} -- Zum Erhalt der Textzeile vertikal um \T{-0.35\textbackslash baselineskip} verschoben.}) und analog\newline \blankcmd{circAT\manArg{(PosX,PosY)}} (\tikz{\circAT{(0,0)};} \footnote{\T{\blankcmd{tikz}\{\blankcmd{circAT}\{(0,0)\};\}}}), sowie \blankcmd{bblock\manArg{(PosX,PosY)}\manArg{text}} (\raisebox{-0.2\baselineskip}{\tikz{\bblock{(0,0)}{42};}} \footnote{\T{\blankcmd{tikz}\{\blankcmd{bblock}\{(0,0)\}\{42\};\}} -- Wieder vertikal um \T{-0.2\textbackslash baselineskip} verschoben.}). Hier fragt man sich nun vielleicht, warum diese nicht in einem entsprechenden Ti\textit{k}Z-Paket sind. Im Rahmen der mit \LILLYxBOXxVersion{1.0.10} eingeführten Modularisierung hat sich diese Verteilung als günstig erwiesen. %% vielleicht doch verschieben?
\newline
%
%
%
...
Zudem definiert sich noch für Tabellen der Befehl \blankcmd{VRule\manArg{width}}, welcher eine Spalte variabler Größe für Tabellen zur Verfügung stellt. Eine exemplarische Einbindung findet sich hier:
\begin{center}
...
Diese Definitionen befinden sich in der Datei: \T{Maths/\_LILLY\_MATHS\_SHORTCUTS}. Sie werden mit \LILLYxBOXxVersion{1.0.10} automatisch mit dem Einbinden von \LILLYxNOTExLibrary{LILLYxMATH} geladen.\medskip
...
Schreibt entsprechend \T{o.B.d.A} (\blankcmd{obda}) und \T{O.B.d.A.} (\blankcmd{Obda}) und beschleunigt damit das Tippen in Beweisen \Smiley.\medskip\newline
%
%
%
...
%
%
\presentCommand[1.0.10]{ceil}[\optArg{math-Ausdruck}\cmdlist \textbackslash floor\optArg{math-Ausdruck}]
Verkürzt das Schreiben von: \clatex{:bs:left:bs:lfloor\<Ausdruck\>:bs:right:bs:rfloor} beziehungsweise \clatex{:bs:lceil}\&\clatex{:bs:rceil} entsprechend:
\begin{multicols}{2}%
    \begin{ditemize}
...
\end{center}
Diese Definitionen befinden sich in der Datei: \T{Maths/\_LILLY\_MATHS\_PLOTS}. Sie werden mit \LILLYxBOXxVersion{1.0.10} automatisch mit dem Einbinden von \LILLYxNOTExLibrary{LILLYxMATH} geladen.\medskip\newline
%
%
%
\presentCommand[1.0.8]{plotline}[\optArg[Ao]{Farbe}\optArg[\textbackslash x]{Variable}\manArg{Term}\optArg[0]{offset}]
Zeichnet in eine \jmark[Graph-Umgebung]{env:graph} eine Funktion (siehe Umgebung für Beispiel). Existiert auch außerhalb von \blankenv{graph}, ist aber hier nur eingeschränkt nutzbar. Mit \emph{offset}${}^{\T{v1.0.10}}$ lässt sich die Funktion entsprechend verschieben.\medskip\newline
%
%
%
...
%
%
\presentCommand[1.0.10]{xmark}[\optArg[x]{text}\manArg{PosX}\optArg[0.15]{linelength}]
...
%
%
\presentCommand[1.0.10]{ymark}[\optArg[xy]{text}\manArg{PosY}\optArg[0.15]{linelength}]
...
\begin{center}
    \begin{tabular}{!{\VRule[1pt]}@{\hspace{0.5em}}C{0.5\textwidth}@{\hspace{0.5em}}|@{\hspace{0.5em}}C{0.4\textwidth}@{\hspace{0.5em}}!{\VRule[1pt]}}
        \specialrule{1pt}{0pt}{0pt}
        {\footnotesize\begin{latex}[breaklines=true]
...
\end{graph}
        \end{latex}} &  \begin{graph}[scale=1.15,maxY=3,numbers]
            \plotline[purple]{sqrt(\x+2.5)};
            \plotline[Ao][\y]{(\y*\y)/1.5};
            \plotseq[Azure]
                {sin(deg(\x))^2};
    ...
    \end{tabular}
\end{center}
Für die \emph{Konfigurationen} gibt es folgende Möglichkeiten:
\begin{center}% \begin{tabularx}{0.75\linewidth}
    \begin{tabularx}{0.7\linewidth}{>{\LILLYxlstTypeWriter}l>{\em}lcl}
        \toprule
        Bezeichner & \normalfont Typ & Standard & Beschreibung\\\midrule
        scale & Zahl & 1 & Skalierungsfaktor \\
        xscale & Zahl & 1 & $x$-Skalierungsfaktor${}^{\T{v1.0.10}}$\\
        yscale & Zahl & 1 & $y$-Skalierungsfaktor${}^{\T{v1.0.10}}$\\
...
\presentEnvironment[1.0.10]{egraph}[\optArg{Konfigurationen}]
...
\end{axis}
\end{tikzternal}
        \end{latex}} &  \begin{tikzternal}[scale=0.6]
