...
Ist \blankcmd{LILLYxDEBUG} auf \blankcmd{true} gesetzt, wird \blankcmd{errorcontextlines} entsprechend modifiziert und eine entsprechende DEBUG-Titlepage gesetzt:\smallskip\\
\makeatletter%
\framebox{%
    \@@Debug@Page@Content%
}~\\

Ist \blankcmd{LILLYxSemester} auf einen Wert größer als $0$ gesetzt, wird automatisch die Konfiguration mittels von \blankcmd{RequestConfig} geladen. Ist weiter \blankcmd{LILLYxBIBTEX} definiert, so werden alle für die Verwendung von $\mathrm{B{\scriptstyle{IB}} \! T\!_{\displaystyle E} \! X}$ % maybe make \BibTex
...
\begin{latex}
\documentclass[Uebungsblatt,Vorlesung=${VORLESUNG},n=${N},Semester=${SEMESTER}]{Lilly}
\begin{document}
...
\begin{center}
    ...
    \end{tabular}
\end{center}
Die Farbe des \T{ALTERNATE}-Designs wird durch \blankcmd{Hcolor} und damit durch die \Jake-\jmark[Einstellung]{mrk:jakesettings} \T{lilly-signatur-farbe} kontrolliert. Beide hier gezeigten Dokumente wurden übrigens nur mit \T{pdflatex} kompiliert! Die Dokumente finden sich im Quellordner der Dokumentation unter \T{Data/Documents/LayoutUebungsblatt}.
...
Setzt im \emph{Hauptteil} einen normalen Start eines neuen Themenbereichs und im \emph{Anhang} einen (im Inhaltsverzeichnis anders aufgeführten) mit \T{Anhang} angeführten neuen Bereich. Der Wechsel zwischen Hauptteil und Anhang erfolgt einmalig durch \blankcmd{startAppendix}.
...
\ilatex{Data/Documents/LayoutZusammenfassung/example-zsf.tex}
    Die Generierung der Titelseite erfolgt übrigens ebenfalls mit \LILLYxNOTExLibrary{LILLYxPHILOSOPHER}. Das (hierbei durch \bbash{pdflatex -shell-escape example-zsf.tex} erzeugte) Ergebnis sieht so aus:
...
Liefert:
\begin{tcbraster}[raster columns=3, blankest,colback=white]
    \tcbincludepdf{Data/Documents/LayoutPlain/example-plain.pdf}
\end{tcbraster}
...
Im Folgenden ein Beispiel, welches sich auch hier (\T{Data/Documents/LayoutPaper/example-paper.tex}) in den Quelldateien der Dokumentation findet. Es erzeugt das folgende Dokument:
\begin{tcbraster}[raster columns=3, blankest,colback=white]
...
Hier ein Beispiel, der dem Code in \T{Data/Documents/LayoutPnP/example-pnp.tex} (der Quelldateien) entspringt:
\begin{tcbraster}[raster columns=2, blankest,colback=white,graphics options={trim=0cm 6.5cm 0cm 0cm,clip}]
    \tcbincludepdf[blankest]{Data/Documents/LayoutPnP/example-pnp.pdf}
\end{tcbraster}
...
Im Folgenden ein Beispiel, welches sich wiedereinmal (\T{Data/Documents/LayoutPoems/poems.tex}) in den Quelldateien der Dokumentation findet. Es erzeugt das folgende Dokument:
\begin{tcbraster}[raster columns=3, graphics pages={1,...,9}, blankest,colback=white]
...
Dieses Paket hat mit \LILLYxBOXxVersion{1.0.8} die Generierung der Titelseiten für Mitschriften und mit \LILLYxBOXxVersion{2.0.0} auch für Zusammenfassungen übernommen. \elable{mrk:philentry}Hier ein kleines Beispiel:
\begin{tcbraster}[raster columns=4, blankest,graphics pages={1,...,8},colback=white]
...
\end{latex}
Dies definiert das Mathe-Siegel, \blankcmd{LILLYxFACULTYxMATHE} ergibt (wird zusätzlich zentriert):
\begin{center}
    \LILLYxFACULTYxMATHE
\end{center}
...
Diese Befehle definieren die Pfade zu den entsprechenden Definitionen. Sie werden jeweils nur dann eingebunden, wenn \blankcmd{LILLYxSemester} einen Wert größer $0$ hält. Hier sind die Standartdefinitionen:
\begin{latex}
\providecommand{\LILLYxColorxTITLExSETTINGSxGENERAL}{%
  \LILLYxPATHxDATA/Semester/\LILLYxSemester/Definitions/GENERAL.tex%
}
\providecommand{\LILLYxColorxTITLExSETTINGSxVORLESUNG}{%
  \LILLYxPATHxDATA/Semester/\LILLYxSemester/Definitions/%
    \LILLYxVorlesung%
}
\end{latex}
...
\def!**!\LILLYxFACULTY{\LILLYxFACULTYxMATHE}
\def!**!\LILLYxFACULTYxCOLOR{FacultyMathexColor}
\def!**!\LILLYxSemester{2}\def!**!\LILLYxVorlesung{ANA1}
...
Offensichtlich wird \blankcmd{LILLYxPHILOSOPHERxBORDERBLOCK} verwendet.
...
Setzt eine Titelseite \emph{ohne} Bonus, so wie sie bereits in der \jmark[Einführung]{mrk:philentry} gezeigt wurden. Das anzuzeigende Titelbild sollte übergeben werden. Es wird auf \blankcmd{LILLYxPHILOSOPHERxBORDERBLOCK} und \blankcmd{LILLYxPHILOSOPHERxMETADATA} zurückgegriffen.
...
Setzt die Titelseite mittels \blankcmd{LILLYxTITLExRAW}, allerdings nur, wenn \blankcmd{LILLYxSemester} größer $0$ ist. In diesem Fall wird allerdings automatisch das dazugehörige Titelbild hinzugefügt.
