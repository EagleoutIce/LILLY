...
if len(sys.argv) > 1 and sys.argv[1] == "prerender":
    load_changes("./changes.save")
else:
    load_changes("./ALLchanges.save")
            ...
            if "-pdf" not in x:  # sloppy :: "Eigene" not in x and
                ...
                if x not in changewatcher or float(changewatcher[x]) < os.path.getmtime(x):
                    ...
                    with open(p, 'w') as out:
                        ...
                        out.write(
                            "\\begin{{preview}}\\getGraphics{{{0}}}\\end{{preview}}\n".format(x))
                        out.write("\\end{document}")
                        out.close()
                    h, t = os.path.split(p)
                    stmt = "jake {0} -lilly-nameprefix: \"\" -lilly-show-boxname: false -lilly-out: \"./{1}/\" -lilly-in: \"./{1}/\"".format(
                        t, h)
                    print(stmt)
                    if os.system(stmt) != 0:
                        print("Error, Exiting...")
                        dump_changes("./changes.save")
                        sys.exit(0)
                    os.remove(p)
                else:
...
with open("./" + fname, 'w') as out:
    ...
    out.write("\\end{document}")
if update:
    # update timestamps if succesful
    if os.system("jake " + fname + " -lilly-nameprefix: \"\" -lilly-show-boxname: false -lilly-compiletimes: 2") == 0:
        for k, v in newchanges.items():
            changewatcher[k] = v
        if outname == "all":  # don't dump for latest
            dump_changes("./ALLchanges.save")
