\RequirePackage[yyyymmdd,hhmmss]{datetime}
\RequirePackage[automark,headsepline]{scrlayer-scrpage}         %Header konfigurieren
\RequirePackage{lastpage}                                       %Letzte Seite erhalten

\providedef{TITLE}{Unbekannte Vorlesung}
\providedef{UEBUNGSHEADER}{\TITLE\\Übungsblatt \LILLY@n}

% Vital configurations
\RequestConfig{\LILLYxPATHxDATA/Semester/Definitions/\LILLYxVorlesung} %Konstanten etc.

% General and Layoutkonfiguration
\def\TUTORBOX#1#2{ %Tutorboxen für Blätter die analog abgegeben werden sollen => !Wird automatisch eingebunden. Siehe weiter unten. Komfort! Yay
\begin{centered}
\noindent\fbox{%
\begin{minipage}{\dimexpr.8\textwidth-2\fboxsep-2\fboxrule\relax}\vspace{.1cm}
\begin{centered}
    \textbf{#2}: \\
    \large{#1}
\end{centered}
\vspace{.1cm}
\end{minipage}}
\end{centered}
\vspace{.5cm}
}

\LILLYcommand\LILLYxLayoutxClear{ %
\pagestyle{empty} %KOMA-FTW!
\pagenumbering{gobble}}

\LILLYcommand\LILLYxLayoutxRestore{ %
\pagenumbering{arabic}
\pagestyle{scrheadings} %KOMA-FTW!
\lohead{\parbox[b][\headheight][b]{.4\textwidth}{\AUTHOR}}
\cohead{\parbox[b][\headheight][b]{.5\textwidth}{\centering\tiny{\UEBUNGSHEADER}}}
\rohead{\parbox[b][\headheight][b]{.3\textwidth}{\raggedleft \heute}} \cofoot{}
\rofoot{\thepage/\pageref{LastPage}}
\ifx\LILLYxUBxSHOWTUTOR\true
    \TUTORBOX{\TUTOR}{Tutor}
\fi}

\AtBeginDocument{\LILLYxLayoutxRestore}
\RequirePackage{float,chngcntr}