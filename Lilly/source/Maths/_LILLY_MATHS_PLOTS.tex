...
\NewDocumentCommand{\xmark}{O{x} m O{0.15}}{\draw (#2,\lillyxPlotxMinX)++(0,#3) -- ++(0,-2*#3) node [below] {\fontsize{\lillyxPlotxLabelSize}{\lillyxPlotxNumSize}\selectfont #1};}
\NewDocumentCommand{\ymark}{O{x} m O{0.15}}{\draw (\lillyxPlotxMinY,#2)++(#3,0) -- ++(-2*#3,0) node [left] {\fontsize{\lillyxPlotxLabelSize}{\lillyxPlotxNumSize}\selectfont #1};}
