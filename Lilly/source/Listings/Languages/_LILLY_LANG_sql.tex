%% Wird für die Dokumentation entsprechend erweitert

\lstdefinelanguage{lSql}{
    language=sql,
    comment=[l]{--},
    %alsoletter={\#,=,-},
    %morecomment=[s]{!}{!},
    escapeinside={!*}{*!},
    showstringspaces=true,  %Damit es keine Verwechslungen gibt
    morekeywords = {WITH,IS,REFERENCES,DEFERRED,OPTION},
    morekeywords = [2]{DOUBLE, REAL, BOOLEAN},
    morekeywords = [3]{},
    morekeywords = [4]{},
    morekeywords = [5]{},
    morekeywords = [6]{},
    morecomment=[s]{/*}{*/}
}

\lstnewenvironment{sql}[1][]
  {\lstset{language=lSql,#1}}
{}
\gdef\csql#1{\LILLYxwriteLst[language=lSql]{#1}}
\gdef\isql#1{\lstinputlisting[language=lSql]{#1}}

\pushList{RegisteredLanguages}{sql/lSql}
