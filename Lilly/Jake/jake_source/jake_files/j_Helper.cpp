...
std::string create_buildrule(const std::string& name_type, const std::string& name_rule, 
                             ...
                             const std::string input_build/*, const std::vector<std::string>& others*/) noexcept {
    ...
    ret_str += std::string("\t") + R"(@$(call LILLYxCompile,$(OUTPUTDIR)$(NAMEPREFIX))" + name_addon + R"(,$(PDFFILE),\\providecommand\\LILLYxMODE{)" 
