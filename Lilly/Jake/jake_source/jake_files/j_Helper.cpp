...
std::string create_buildrule( const std::string& name_type, const std::string& name_rule, const std::string& mode, bool complete/*, const std::vector<std::string>& others*/) noexcept {
    ...
    ret_str += std::string("\t") + R"($(call LILLYxCompile,$(OUTPUTDIR)$(NAMEPREFIX)$(PDFFILE),\\providecommand\\LILLYxMODE{)" + name_rule + R"(}\\providecommand\\LILLYxMODExEXTRA{)" + (complete?"TRUE":"FALSE") + R"(})))";
    return ret_str;
}
